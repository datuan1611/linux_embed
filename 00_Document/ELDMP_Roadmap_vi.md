...
|---|---|---|
| 3 | Breadboard, dây jumper (M–M và M–F), vài LED, điện trở 220–330Ω, vài điện trở 10kΩ, vài nút bấm tactile, thẻ nhớ microSD (Class 10+, 8–16GB), cáp USB-to-TTL serial **3.3V** | GPIO output/input cho Lab 0 và phần driver; thẻ microSD dùng luôn làm bản backup nêu ở mục 16; cáp serial là cách duy nhất xem log boot sớm nếu board không lên được qua mạng/USB — lưới an toàn quan trọng cho Sprint 3 và cả Sprint 9 |
| 10 | Một động cơ DC nhỏ, một IC/breakout driver động cơ (ví dụ L298N hoặc DRV8833), một cảm biến đơn giản (ví dụ biến trở, cảm biến nhiệt độ DS18B20, hoặc cảm biến khoảng cách hồng ngoại) | Phần xây vòng lặp đọc cảm biến + điều khiển motor/PWM |
| 11 | Hai module breakout CAN transceiver (ví dụ MCP2551 hoặc SN65HVD230), mỗi board một cái | Cả BeagleBone Black lẫn STM32 Nucleo đều không có sẵn CAN transceiver trên board — chỉ có CAN controller; cần transceiver để đưa tín hiệu ra bus CAN vật lý |

**Quan trọng:** adapter USB-to-TTL phải là mức logic **3.3V**, không
phải 5V/RS232 — chân UART của BeagleBone Black không chịu được 5V, cắm
nhầm adapter 5V có thể làm hỏng board.
...
- **Lab 0 (thử nghiệm, ~3–5 ngày, warm-up phần cứng trước khi đụng code kernel):** viết một chương trình C ở user-space mở `/dev/mem`, `mmap()` vùng thanh ghi GPIO, và bật/tắt một chân bằng cách đọc/ghi thanh ghi trực tiếp — không qua kernel API, không qua HAL. Bài này xây trực giác thanh ghi/bitwise mà driver kernel cần, trên hardware thật, không đụng code kernel và không cần board MCU. **Lưu ý:** một chân phải được mux đúng sang chế độ GPIO thì việc ghi thanh ghi trực tiếp mới có tác dụng — dùng tiện ích `config-pin` hoặc kiểm tra chế độ của chân trong Technical Reference Manual của AM335x trước khi vội kết luận việc ghi thanh ghi bị lỗi.
