...
open()
	...
	pathname:
		...
		tương đối	:path từ thư mục đang làm việc
	return:
		OK			:con trỏ file fd, giá trị int >= 3 (khi init, OS mở sẵn 3 file stdin, stdout, stderror)
		NG			:giá trị < 0

fflush()
    CPU đẩy dữ liệu vào bộ nhớ đệm (thường là RAM), khu vực nhớ: DMA, cache phụ
...
.memory leak
	check thông tin ở file /proc/meminfo:
		MemTotal		: tổng mem của RAM
		MemFree			: mem chưa được dùng
		MemAvailable	: mem có thể sử dụng để cấp phát luôn
		Cached			: mem dùng để cached file
	khi bị memory leak:
		MemAvailable	-> giảm nhanh, liên tục sau 1 thời gian
		MemFree			-> giảm dần
		Cached			-> tăng không nhiều
	trường hợp KHÔNG phải memory leak:
		MemAvailable	-> giảm không nhiều, gần như không đổi
		MemFree			-> giảm dần
		Cached			-> tăng nhanh, nhiều
```
