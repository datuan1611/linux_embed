...
.trước khi vào hàm main, kernel sẽ chạy 1 số đoạn code start routine để khởi tạo bộ nhớ cho process
	khi hàm main start, nó sẽ nhận được 2 tham số từ kernel
	tham số argv[0] luôn là đường dẫn path của chương trình
	--------------------------
	gcc -g main.c -o app
	./app hello world
	argv[0]="./app"
...
=> dựa vào mã trả về khi kết thúc của chương trình -> có thể phán đoán được nguyên nhân chương trình vừa crash
```
...
.tạo ra 1 bản sao cho bộ nhớ hiện tại của process
	PID thật sẽ được trả cho process con (trong luồng chạy của process cha)
	PID 0 sẽ được trả về cho process cha (trong luồng chạy của process con)
...
.khởi tạo process mới từ chương trình nằm tại pathname
 sau khi gọi hàm execl, toàn bộ source code phía sau của chương trình sẽ không được thực hiện nữa
.execl sẽ thay không gian bộ nhớ của process hiện tại bằng new process image
