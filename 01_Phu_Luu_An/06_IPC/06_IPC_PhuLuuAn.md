...
from Phú Lưu An:
- [ipc introduction](https://www.youtube.com/watch?v=EOEmic-870k "https://www.youtube.com/watch?v=EOEmic-870k")
- [signal](https://www.youtube.com/watch?v=ViWv0t8U_-8 "https://www.youtube.com/watch?v=ViWv0t8U_-8")
- [shared memory](https://www.youtube.com/watch?v=RP-PIdqnqiY "https://www.youtube.com/watch?v=RP-PIdqnqiY")
- [socket](https://www.youtube.com/watch?v=-Fs6wAV7tEw "https://www.youtube.com/watch?v=-Fs6wAV7tEw")
...
**Signal**
- cơ chế gửi nhận tín hiệu,
	+ có thể gửi tới bất kỳ process nào đó
	<br>có thể xử lý signal bất thường từ hệ thống
	<br>có 32 giá trị define, trong đó có 4 giá trị trống có thể viết hàm handle để xử lý riêng

- 1 process có 1 không gian bộ nhớ VAS
	+ ở đầu VAS có 1 vùng memory PD (process descriptor) chứa các thông tin của process,
	<br>trong PD chứa 1 bảng signal_handler(), có 32 hàng, mỗi hàng có index hàng + địa chỉ hàm handle() cho signal
	<br>khi đăng ký 1 signal = đăng ký con trỏ hàm handler() vào bảng để xử lý cho signal đấy
	<br>nên bảng signal_handler chỉ có tác dụng với process có PD đó thui
	+ ở bảng signal_handler có:
		...
		<br>(đa số các ô không cho phép ghi đè, ví dụ SIGKILL)
...
### 0.2. IPC for uer space - kernel space
...
- độ phổ biến:
	+ device file > ioctl > procfs > sysfs > system call > socket
	<br>(socket chậm nên ít dùng giữa user-kernel space)
...
- file sysfs có thể đọc/ghi 1 cách thụ động (với procfs thì phải implement hàm đọc/ghi)
	...
	<br>driver phải compare giá trị trước/sau ở sysfs để check user có ghi giá trị mới ở sysfs hay không
...
<br>procfs: muốn tạo file nào thì user phải tự tạo file đó
...
- khi viết driver cho màn hình
	...
	<br>đẩy data -> ép read/write qua device file
...
.1 số signal cơ bản
	...
	SIGSEGV		//khi phát sinh lỗi segment fault
